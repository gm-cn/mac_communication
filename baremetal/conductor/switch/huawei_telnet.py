...
class HuaweiSwitch(models.ModelBase):

    def __init__(self, username, password, host, port=23, timeout=180):
        super(HuaweiSwitch, self).__init__()
        self.host = host
        ...
        def _save():
            try:
                ...
                if "Error" in error_msg:
                    ...
                    raise exceptions.ConfigSwitchError(command='save', error=result)
            except Exception:
                raise
            return error_msg
    ...
    def _execute_relative(self, command):
        ...
        try:
            with sw_lock.PoolLock(self.locker, **self.lock_kwargs):
                ...
                for i in command:
                    net_connect.write((i + '\n').encode('utf-8'))
                result = net_connect.read_until("Error", timeout=3)
    ...
    def _clean_all_config(self, switch, template_name=None):
        ...
        all_ports_cmd = ['system-view immediately']
    ...
    def clean_all_config(self, switch, template_name=None):
        clean_cmd_set = self._clean_all_config(switch, template_name) + [ 'q']
    ...
    def init_dhclient_config(self, switch, clean_cmd_set=[]):
        ...
        init_dhclient_cmds = set_vlan_cmd + [ 'q']
    ...
    def init_all_config(self, switch, template_name, is_dhclient):
        ...
        create_template_cmd = ["qos car %s cir %s kbps" % (template_name, cir), ""]
        ...
        init_cmd_set = create_template_cmd + all_ports_cmd + [ 'q']
    ...
    def open_port(self, ports):
        open_cmd = ["system-view immediately"]
        for port in ports:
            open_cmd += ["interface " + port, "undo shutdown",  "q"]
        commands = open_cmd + ["q"]
        logger.debug("open ports command:%s" % commands)
        return self._execute(commands)

    def shutdown_port(self, ports):
        shutdown_cmd = ["system-view immediately"]
        for port in ports:
            shutdown_cmd += ["interface " + port, "shutdown",  "q"]
        commands = shutdown_cmd + ["q"]
        logger.debug("close ports command:%s" % commands)
        return self._execute(commands)

    def create_limit_template(self, templates):
        create_command = ["system-view immediately"]
    ...
    def delete_limit_template(self, templates):
        delete_command = ["system-view immediately"]
        for template in templates:
    ...
    def unset_limit(self, inbound_ports, outbound_ports):
        ...
        for port in inbound_ports:
            inbound_cmd += ["interface " + port, "undo qos car inbound",  "q"]
        outbound_cmd = []
        for port in outbound_ports:
            outbound_cmd += ["interface " + port, "undo qos lr outbound",  "q"]
    ...
    def get_relations(self, special_vlan=None, special_mac=[]):
        ...
        if len(special_mac) > 0:
            for item in special_mac:
                datas = self._execute_relative(["display mac-address %s" % item])
        ...
        if special_vlan:
            datas = self._execute_relative(["display mac-address vlan %s" % special_vlan])
            for line in datas.split("\n")[12:-3]:
                data = pattern.findall(line)
                mac = ":".join(i[0:2] + ":" + i[2:4] for i in data[0].split("-"))
                relations.append({"mac": mac, "port": data[2]})
...
class SwitchPlugin(object):
    ...
    def set_vlan(self, req):
        ...
        with HuaweiSwitch(body.username, body.password, body.host) as client:
            try:
    ...
    def init_all_config(self, req):
        ...
        for switch in body.switches:
            with HuaweiSwitch(switch.username, switch.password, switch.host) as client:
                try:
